...
def init_vib(args):
    if args.dataset == 'MNIST':
        ...
        encoder = resnet18(3, args.dimZ * 2)  # resnet18(1, 49*2)
        decoder = LinearModel(n_feature=args.dimZ, n_output=3 * 96 * 96)
        lr = args.lr
...
def construct_input(dataset, dataloader_target_with_tri_tar, target_vib, model, loss_fn, args, epoch):
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)

    temp_img = torch.empty(0, 3, 96, 96).float().to(args.device)
    temp_label = torch.empty(0).long().to(args.device)

    for step, (target_x, target_y) in enumerate(dataloader_target_with_tri_tar):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        x2 = x
        x3=x
        for idx in range(x3.size(0)):
            if idx == 0: #y_rate[idx]
                x3[idx] =  target_x # x2[idx] / 4 +
                y[idx] = target_y
...
args.dimZ = 128  # 40 # 2
args.batch_size = 2
args.erased_local_r = 0.02  # when ratio value is 0.000017, the sample is 1. 0.002 # the erased data ratio
...
remaining_size = full_size_w_o_target - erasing_size
remaining_set, erasing_set = torch.utils.data.random_split(full_w_o_target_set,
                                                           [remaining_size, erasing_size])

# the construction data size
construct_size = int(full_size * args.construct_size) #16  #
remaining_w_o_cons_size = remaining_size - construct_size
remaining_set_w_o_cons, construct_set = torch.utils.data.random_split(remaining_set,
                                                                      [remaining_w_o_cons_size, construct_size])




# this is the final remaining dataset, without the erasing dataset, and the auxiliary dataset
print(len(remaining_set_w_o_cons))
print('erased size', len(erasing_set))
# print(len(remaining_set_w_o_twin.dataset.data))


# it is the final remaining dataset
dataloader_remaining_w_o_cons = DataLoader(remaining_set_w_o_cons, batch_size=args.batch_size, shuffle=True)
...
# Filter the dataset to find indices of all samples with label == 1
# indices_for1 = [i for i, (img, label) in enumerate(train_set) if label == 1][:10]

# Create a subset from these indices
# filtered_subset_for1 = Subset(train_set, indices_for1)

# Create a DataLoader from the subset
#filtered_loader = DataLoader(filtered_subset_for1, batch_size=10, shuffle=True, num_workers=1)



# it is the target dataset

# Now create the dataset with modified labels
target_set_modified = MNISTTargetLabelDataset(target_set, target_label=2)

construct_set_modified = MNISTTargetLabelDataset(construct_set, target_label=2)

dataloader_target_only = DataLoader(target_set, batch_size=args.batch_size, shuffle=True)


dataset1 = dataloader_remaining_w_o_cons.dataset
dataset2 = dataloader_erasing.dataset
dataset3 = dataloader_constructing.dataset
dataset_target = dataloader_target_only.dataset


# Concatenate datasets and create a new loader, this is just an example to show how to concatenate the datasets
combined_dataset = ConcatDataset([dataset2, dataset3])
...
dataloader_target_with_tri = DataLoader(target_with_tri, batch_size=args.batch_size, shuffle=True)
dataloader_target_with_tri_tar = DataLoader(target_with_tri_tar, batch_size=args.batch_size, shuffle=True)


combined_target = ConcatDataset([construct_set_modified, target_set_modified])
dataloader_target = DataLoader(combined_target, batch_size=args.batch_size, shuffle=True)

#dataset_target

combined_dataset = ConcatDataset([dataset1, dataset2, dataset3]) # with original dataset2, to show the noise extent. erasing_with_tri
...
vib.eval()
acc_r = eva_vib(vib, dataloader_erasing, args, name='on clean remaining dataset', epoch=999)
backdoor_acc = eva_vib(vib, dataloader_target_with_tri_tar, args, name='on dataloader_target_with_tri_tar', epoch=999)
acc = eva_vib(vib, test_loader, args, name='on test dataset', epoch=999)



# we need to update for the target training

vib_adv_target = copy.deepcopy(vib)

start_time = time.time()
for epoch in range(20):
    vib_adv_target.train()
    vib_adv_target = vib_train(dataloader_target_with_tri, vib_adv_target, loss_fn, reconstruction_function, args, epoch,
                    train_type)  # dataloader_total, dataloader_w_o_twin
...
print(f'VIB Training took {running_time} seconds')
vib_adv_target.eval()
acc_r = eva_vib(vib_adv_target, dataloader_erasing_with_tri, args, name='on dataloader_erasing_with_tri data', epoch=999)
acc_r = eva_vib(vib_adv_target, dataloader_target_with_tri_tar, args, name='on dataloader_target_with_tri_tar', epoch=999)
acc_r = eva_vib(vib_adv_target, dataloader_target_with_tri, args, name='on dataloader_target_with_tri', epoch=999)
...
# constructing with perturbed unlearning data with unlearning algorithms
...
for epoch in range(args.num_epochs+2):
    dataloader_constructing1, fixed_vib = construct_input(dataloader_constructing1, dataloader_target_with_tri_tar, vib_adv_target,
                                                            fixed_vib,
                                                            loss_fn, args,
                                                            epoch)
    # vib.train()
...
start_time = time.time()
unlearned_vib = prepare_unl(dataloader_constructing1, combined_loader, unlearned_vib, loss_fn, args, "no noise")
...
unlearned_vib.eval()
acc_r = eva_vib(unlearned_vib, dataloader_target, args, name='on target data', epoch=999)
acc_r = eva_vib(unlearned_vib, dataloader_target_only, args, name='on target only data', epoch=999)
acc_r = eva_vib(unlearned_vib, dataloader_target_with_tri_tar, args, name='on dataloader_target_with_tri_tar', epoch=999)


con_acc = eva_vib(unlearned_vib, dataloader_constructing1, args, name='unlearned model on constructing data', epoch=999)
backdoor_acc = eva_vib(unlearned_vib, dataloader_erasing_with_tri, args, name='unlearned model on erased data', epoch=999) ## erased samples_with_trigger should be included
...
acc_r = eva_vib(unlearned_vib2, dataloader_target_with_tri_tar, args, name='on dataloader_target_with_tri_tar',
                epoch=999)

con_acc = eva_vib(unlearned_vib2, dataloader_constructing1, args, name='unlearned model on constructing data',
                  epoch=999)
backdoor_acc = eva_vib(unlearned_vib2, dataloader_erasing_with_tri, args, name='unlearned model on erased data',
                       epoch=999)  ## erased samples_with_trigger should be included
