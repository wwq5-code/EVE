...
args.dimZ = 128  # 40 # 2
args.batch_size = 160
args.erased_local_r = 0.006  # when ratio value is 0.000017, the sample is 1. 0.002 # the erased data ratio
...
#dataloader_target_only_label_modified = DataLoader(target_set_modified, batch_size=args.batch_size, shuffle=True)
...
print(f'VIB Training took {running_time} seconds')
vib_adv_target.eval()
#acc_r = eva_vib(vib_adv_target, dataloader_target, args, name='on target data', epoch=999)
acc_r = eva_vib(vib_adv_target, dataloader_target_with_tri_tar, args, name='on dataloader_target_with_tri_tar', epoch=999)
#acc_r = eva_vib(vib_adv_target, dataloader_target_only_label_modified, args, name='on modified target only data', epoch=999)
...
start_time = time.time()
unlearned_vib = prepare_unl(dataloader_constructing1, dataloader_remaining_w_o_cons, unlearned_vib, loss_fn, args, "no noise")
