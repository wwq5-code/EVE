...
indices_for1 = [i for i, (img, label) in enumerate(train_set) if label == 1][:100]  #100
...
combined_dataset = ConcatDataset([dataset2, filtered_subset_for1])
...
start_time = time.time()
unlearned_vib = prepare_unl(dataloader_constructing1, combined_loader, unlearned_vib, loss_fn, args, "no noise") #dataloader_remaining_w_o_cons
